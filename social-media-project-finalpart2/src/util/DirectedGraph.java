...
public class DirectedGraph<T> implements GraphInterface<T>
{
    ...
    public Iterator<VertexInterface<T>> getVerticesValueIterator() {
...
// If nextVertex is unvisited AND has only visited neighbors)
    ...
    public boolean addVertex(T vertexLabel) {
        ...
        if (vertices.contains(vertexLabel)) return false;
    ...
    /**
     * Removes a given vertex to this graph.
     *
     * @param vertexLabel An object that labels the vertex and is
     *                    distinct from the labels of current vertices.
     * @return True if the vertex is removed, or false if not.
     */
    @Override
    public boolean removeVertex(T vertexLabel) {
        ...
        Iterator<VertexInterface<T>> iter = vertices.getValueIterator();

        while (iter.hasNext()) {
            VertexInterface<T> nextVertex = iter.next();

            if (hasEdge(nextVertex.getLabel(), vertexLabel))
                removeEdge(nextVertex.getLabel(), vertexLabel);

            if (hasEdge(vertexLabel, nextVertex.getLabel()))
    ...
    /**
     * Removes a desired edge from the graph. The edge must exist in
     * the graph in order to be removed
     *
     * @param begin An object that labels the origin vertex of the edge.
     * @param end   An object, distinct from begin, that labels the end
     *              vertex of the edge.
